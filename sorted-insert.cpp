...
bool JobQueue_AddJob(JobQueue& jobQueue, double offsetMs, Callback callback)
{
  ...
  if (jobQueue.jobCount < jobQueue.maxJobs) {
    ...
    callback();
    // times must always be in the future if they are to be safe to add while
    // JobQueue_Update is running.
    if (offsetMs <= 0) {
      offsetMs = 1;
    }
    ...
    if (job.whenMs >= jobQueue.lastJobTime) {
      ...
      std::cout << "new job is lower than exisiting jobs, current count: " << jobQueue.jobCount << std::endl;
      std::memmove(&jobQueue.jobs[1], &jobQueue.jobs[0], jobQueue.jobCount * jobSize);
      jobQueue.jobs[0] = job;
      jobQueue.nextJobTime = job.whenMs;
    } else {
      std::cout << "new job is in the middle" << std::endl;
...
// Theoretically inserts are safe unless they insert with 0 time. If we force
// all inserts to be at least 1ms after jobQueue.currentTimeMs newly inserted
// jobs will have to be after the jobs that we are assured we are only deleting
// old jobs. This does mean that while executing callbacks added jobs will see
// the old size of the array.
void JobQueue_Update(JobQueue& jobQueue, float _timeSinceLastUpdateMs)
{
  ...
  if (jobQueue.currentTimeMs >= jobQueue.nextJobTime) {
    ...
    if (remaining != jobQueue.jobCount) {
      if (remaining != 0) {
        ...
        std::memmove(&jobQueue.jobs[0],
                     &jobQueue.jobs[jobQueue.jobCount - remaining],
                     remaining * jobSize);
